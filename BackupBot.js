...
module.exports = {
  ...
  BotHandler: (target, mod, commandName, parse) => {
    if(bottressDown && (target == '#thabuttress' || target == '#joefish5')){
      switch(commandName){
        case 'discord':
          return `All people are welcome to join the discord, but subs get access to special channels!!! https://discord.gg/j3G5bx3`
        case 'house':
        ...
        switch(commandName){
          case 'buttshout':
            return co(function *() {
              ...
              let message = `buttHype Be sure to follow ${json.display_name}! Their last stream was ${json.game} so be sure to check them out!!! ${json.url.replace(' https://www.', '')}`
